...
la catégorie de produit => CategoryName dans Categories */
	...
	SELECT products.ProductName AS 'Nom du produit', suppliers.CompanyName AS 'Nom du fournisseur', categories.CategoryName AS 'Catégorie du produit' FROM products, suppliers, categories WHERE products.ProductID = 1 AND (products.CategoryID = categories.CategoryID) AND (products.SupplierID = suppliers.SupplierID);

/* Plus d'infos */
        SELECT products.ProductName AS 'Nom du produit', suppliers.CompanyName AS 'Nom du fournisseur', categories.CategoryName AS 'Catégorie du produit', products.UnitPrice AS 'Prix du produit', suppliers.City AS 'Ville du fournisseur', suppliers.Country AS 'Pays du fournisseur'
...
/*exercice 1: 
Afficher le nom, le prénom, la date de naissance de tous les employés qui ont comme titre "Sales Representative" ? */
        SELECT FirstName, LastName, BirthDate FROM `employees` WHERE Title = "Sales Representative";

/*exercice 2: 
Où travaille Janet Leverling (Nom du territoire et de la région) ?
...
/* Tous les clients de Londres (nom de la société, nom de la personne de contact, n° de telephone) classés par ordre alphabétique ? */
...
/*Quel employé a réalisé la commande 10256, avec quel transporteur et pour quel client ?
