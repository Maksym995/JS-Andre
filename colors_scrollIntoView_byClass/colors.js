...
let titresDefilables = document.querySelectorAll(".titresDefilables")

function scrollToElement() {this.scrollIntoView({behavior: 'smooth', block: 'start'})

}

for (let i = 0; i< titresDefilables.length; i++) {
  titresDefilables[i].addEventListener('click', scrollToElement);
...
let titre = document.querySelector(".titresDefilables")

function goToScrollEvent(e) {
  e.preventDefault();
  titre.scrollIntoView({behavior: 'smooth', block: 'start'}) 
}

document.addEventListener("scroll", goToScrollEvent);
